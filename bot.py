...
async def ensure_user(conn, tg_id: int, name: str):
    ...
    if row: return row[0]
    row = await q1(conn, "INSERT INTO tennis.users(phone, tg_id, name) VALUES (%s,%s,%s) RETURNING id",
...
async def open_session(tg_user_id: int, m: Message|None, c: CallbackQuery|None, sid: int):
    ...
    if m: await m.answer(text, reply_markup=markup)
    else: await c.message.edit_text(text, reply_markup=markup)
...
async def cb_cancel(c: CallbackQuery):
    ...
    async with psycopg.AsyncConnection.connect(DATABASE_URL) as conn:
        ...
        row = await q1(conn,
            ...
            uid, sid)
        if not row:
            await c.answer("У вас нет активной записи.", show_alert=True); return
        bid = row[0]
